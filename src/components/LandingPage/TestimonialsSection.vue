...
<template>
    <div class="bg-gradient-to-b from-[#11263B] to-[#132b42] text-center text-white py-5 px-12 relative">
        ...
        <div v-for="(testimonial, index) in testimonials" :key="index" :class="slide === index ? 'block' : 'hidden' ">
            ...
            <div class="flex flex-row gap-5 w-fit mx-auto mt-5">
                <div class="h-[20px] w-[20px] rounded-[50%] border border-2 border-red-700" @click="slide = index" :class="slide === index ? 'bg-white' : 'bg-none' " v-for="(testimonial, index) in testimonials" :key="index"></div>
            </div>
        </div>
